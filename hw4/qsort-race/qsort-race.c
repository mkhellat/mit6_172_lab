...
void quickSort(data_t arr[], int l, int h) {
    if (l < h) {
        ...
        // The race occurred because both spawned calls accessed shared state
        // (the global comparison_count variable) without synchronization.
        // By spawning only one call and executing the other serially, we
        // maintain parallelism while eliminating the determinacy race.
        cilk_spawn quickSort(arr, l, p - 1);
        quickSort(arr, p + 1, h);  // Execute serially - no shared state access
        cilk_sync;
    }
}
