...
plt.tight_layout()
plt.savefig('plots/t4_t10_compatible_karan.png', dpi=300, bbox_inches='tight')
print("Plot 1 saved: plots/t4_t10_compatible_karan.png")
...
# T_64 constraints
T1_T64_work = np.full_like(T_inf, 640)
T1_T64_greedy = 640 - 63 * T_inf
...
ax2.plot(T1_T64_work, T_inf, 'r--', label='T₆₄ Work Law: T₁ ≤ 640', linewidth=2)
ax2.plot(T1_T64_greedy, T_inf, 'r-', label='T₆₄ Greedy Bound: T₁ ≥ 640 - 63T∞', linewidth=2)

# Span Law constraints
ax2.axhline(y=80, color='b', linestyle=':', alpha=0.5, label='T₄ Span Law: T∞ ≤ 80')
ax2.axhline(y=10, color='r', linestyle=':', linewidth=3, label='T₆₄ Span Law: T∞ ≤ 10 ⭐', alpha=0.7)

# Feasible region when T∞ ≤ 10 (tightest from T_64)
T_inf_constrained = np.linspace(0, 10, 1000)
T1_T4_feasible_low = 320 - 3 * T_inf_constrained
T1_T4_feasible_high = np.full_like(T_inf_constrained, 320)
T1_T64_feasible_low = 640 - 63 * T_inf_constrained
T1_T64_feasible_high = np.full_like(T_inf_constrained, 640)

# Check: T₁ ≥ max(320-3T∞, 640-63T∞), T₁ ≤ 320
# At T∞ = 10: T₁ ≥ max(290, 10) = 290, T₁ ≤ 320
# So: 290 ≤ T₁ ≤ 320 is feasible
...
ax2.fill_betweenx(T_inf_constrained,
                 ...
                 alpha=0.3, color='purple', label='Feasible Region: 290 ≤ T₁ ≤ 320 (when T∞ ≤ 10)')

ax2.axvline(x=290, color='purple', linestyle='--', alpha=0.5)
ax2.axvline(x=320, color='purple', linestyle='--', alpha=0.5)
ax2.text(305, 5, '✓ COMPATIBLE\n290 ≤ T₁ ≤ 320\nwhen T∞ ≤ 10', 
        bbox=dict(boxstyle='round', facecolor='green', alpha=0.7, edgecolor='black'),
        fontsize=12, ha='center', color='white', weight='bold')
...
plt.tight_layout()
plt.savefig('plots/t4_t64_compatible_karan.png', dpi=300, bbox_inches='tight')
print("Plot 2 saved: plots/t4_t64_compatible_karan.png")
...
ax3.plot(T1_T64_work, T_inf, 'r--', label='T₆₄ Work Law: T₁ ≤ 640', linewidth=2)
ax3.plot(T1_T64_greedy, T_inf, 'r-', label='T₆₄ Greedy Bound: T₁ ≥ 640 - 63T∞', linewidth=2)

# Span Law constraints
ax3.axhline(y=42, color='g', linestyle=':', alpha=0.5, label='T₁₀ Span Law: T∞ ≤ 42')
ax3.axhline(y=10, color='r', linestyle=':', linewidth=3, label='T₆₄ Span Law: T∞ ≤ 10 ⭐', alpha=0.7)
...
# Check: T₁ ≥ max(420-9T∞, 640-63T∞), T₁ ≤ 420
# At T∞ = 10: T₁ ≥ max(330, 10) = 330, T₁ ≤ 420
# So: 330 ≤ T₁ ≤ 420 is feasible
...
ax3.fill_betweenx(T_inf_constrained,
                 ...
                 alpha=0.3, color='purple', label='Feasible Region: 330 ≤ T₁ ≤ 420 (when T∞ ≤ 10)')

ax3.axvline(x=330, color='purple', linestyle='--', alpha=0.5)
ax3.axvline(x=420, color='purple', linestyle='--', alpha=0.5)
ax3.text(375, 5, '✓ COMPATIBLE\n330 ≤ T₁ ≤ 420\nwhen T∞ ≤ 10', 
        bbox=dict(boxstyle='round', facecolor='green', alpha=0.7, edgecolor='black'),
        fontsize=12, ha='center', color='white', weight='bold')
...
plt.tight_layout()
plt.savefig('plots/t10_t64_compatible_karan.png', dpi=300, bbox_inches='tight')
print("Plot 3 saved: plots/t10_t64_compatible_karan.png")
